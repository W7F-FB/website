...
import { P, List } from "@/components/website-base/typography"
...
export function getTicketingFaqs(): FAQItem[] {
    const ticketingSection = faqSections.find(section => section.id === "ticketing")
    return ticketingSection?.items ?? []
}

export function getCompetitionFormatFaqs(): FAQItem[] {
    const section = faqSections.find(section => section.id === "competition-format")
    return section?.items ?? []
}

export function getEventFaqs(): FAQItem[] {
    const section = faqSections.find(section => section.id === "event")
    return section?.items ?? []
}
