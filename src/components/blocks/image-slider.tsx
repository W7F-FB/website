...
type ImageSliderProps = React.ComponentProps<typeof Carousel> & {
  autoplay?: boolean
  autoplayDelay?: number
}

function ImageSlider({ className, children, autoplay = false, autoplayDelay = 5000, ...props }: ImageSliderProps) {
  ...
  return (
    <Carousel
      className={cn("w-full h-full relative [&>[data-slot='carousel-content']]:h-full [&>[data-slot='carousel-content']]:w-full", className)}
      opts={{
        watchDrag: false,
        loop: true,
        ...props.opts
      ...
      {...props}
    >
      <CarouselContent className="!gap-0 !m-0 !w-full !h-full relative">
        {children}
      </CarouselContent>
      <div className="absolute bottom-0 right-0 mb-2 mr-2 z-[20]">
        <div className="flex">
          <ImageSliderPrevious className="-mr-[0.5px] z-1 hover:z-2" />
          <ImageSliderNext className="-ml-[0.5px] z-1 hover:z-2" />
        </div>
...
function ImageSliderSlide({ className, ...props }: React.ComponentProps<typeof CarouselItem>) {
  return (
    <CarouselItem
      className={cn("relative overflow-hidden basis-full w-full h-full !m-0 !p-0 ", className)}
      {...props}
    />
  )
}

function ImageSliderPrevious({ className, ...props }: React.ComponentProps<typeof CarouselPrevious>) {
  return (
    <CarouselPrevious
...
function ImageSliderNext({ className, ...props }: React.ComponentProps<typeof CarouselNext>) {
  return (
    <CarouselNext
      className={cn("relative inset-auto h-16 w-16 rounded-none translate-x-0 translate-y-0 !bg-background/40 hover:!bg-background/70 backdrop-blur-sm", className)}
      {...props}
...
export {
  type CarouselApi as ImageSliderApi,
  ImageSlider,
  ImageSliderSlide,
}
